...
namespace Routes { // All views are for external use (+ API)
    ...
    const json predictRateOfFuel(const std::string& username, const std::string& password, const double gallonsRequested, const double companyProfitMargin); // Calls `FuelQuote` constructor, calls `Client` constructor, call functions in classes that update the database, return json of `Client` and `FuelQuote`
